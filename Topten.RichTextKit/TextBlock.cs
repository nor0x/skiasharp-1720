...
namespace Topten.RichTextKit
{
    ...
    public class TextBlock : StyledText
    {
        ...
        /// <summary>
        /// Create numbering list run
        /// </summary>
        /// <param name="number">The number to be used in the run</param>
        public FontRun CreateNumberingListRun(int number)
        {
            var slice = new Utf32Buffer($"{number}.").AsSlice();
            // assume we have one run (at least the one corresponding to \u2029)
            var fontRun = FontRuns.First();
            var run = CreateFontRun(fontRun.StyleRun, slice, _resolvedBaseDirection, fontRun.Style, fontRun.Typeface, fontRun.Typeface);
            run.Line = fontRun.Line;
            
            run.MoveGlyphs(0, fontRun.Line.BaseLine);
            return run;
        }
