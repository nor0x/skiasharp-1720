...
namespace Topten.RichTextKit.Editor
{
    ...
    public partial class TextDocument
    {
        ...
        {
            ReplaceText(view, range, codePoints, null, semantics);
        }


        public void ReplaceText(ITextDocumentView view, TextRange range, Slice<int> codePoints, IStyle style, EditSemantics semantics)
        {
            ...
            ReplaceTextInternal(view, range, new StyledText(codePoints, style), semantics, -1);
        }
        ...
        public int GetParagraphForCodePointIndex(CaretPosition position, out int indexInParagraph)
        {
        ...
        public void Layout()
        {
            ...
            // Layout paragraphs
            SKColor bulletColor = SKColor.Empty;
            for (int i = 0; i < _paragraphs.Count; i++)
            {
                ...
                para.ContentYCoord = yCoord + prevYMargin;

                if (para.ListKind == ListKind.NumberingList)
                {
                    var number = para.NumberedList.Count(p => para.NumberedList.IndexOf(p) < para.NumberedList.IndexOf(para) && p.FirstParaInList) + 1;
                    var paraNumberedListRun = para.TextBlock.CreateNumberingListRun(number);
                    para.NumberedListRun = paraNumberedListRun;
                    
                    para.NumberingListContentXCoord = para.ContentXCoord;
                    para.ListContentXCoord = Math.Max(paraNumberedListRun.Width, para.ListTextOffset >= 0 ? para.ListTextOffset : DefaultNumberedListParagraphOffset);
                    para.ContentXCoord += para.ListContentXCoord;
                }
                else if (para.ListKind == ListKind.BulletList)
                {
                    if (i == 0 || _paragraphs[i - 1].ListKind != ListKind.BulletList)
                    {
                        var fontRun = para.TextBlock.FontRuns.First();
                        bulletColor = fontRun.Style.TextColor;
                    }

                    var textLine = para.TextBlock.Lines.First();
                    var radius = textLine.Height / 8;
                    para.BulletColor = bulletColor;
                    para.BulletCenterXCoord = para.ContentXCoord + 3 * radius / 2;
                    para.BulletCenterYCoord = para.ContentYCoord + radius * 4;
                    para.BulletRadius = radius;
                    para.ListContentXCoord = Math.Max(2 * radius, para.ListTextOffset >= 0 ? para.ListTextOffset : DefaultBulletListParagraphOffset);
                    para.ContentXCoord += para.ListContentXCoord;
                }
                ...
                _measuredHeight += para.ContentHeight;
        ...
        public float DefaultBulletListParagraphOffset { get; set; } = 30;

        public float DefaultNumberedListParagraphOffset { get; set; } = 30;
        ...
        void ReplaceTextInternal(ITextDocumentView view, TextRange range, StyledText text, EditSemantics semantics, int imeCaretOffset)
        {
            ...
            using (_undoManager.OpenGroup(group))
            {
                ...
                if (ShouldProcessListParagraph(range, text, semantics, ref paragraphIndex))
                {
                    
                    _undoManager.Do(new UndoCancelList(new Tuple<int, int>(paragraphIndex, paragraphIndex)));
                }
                else
                {
                    // Delete range (if any)
                    ...
                    if (text.Length != 0)
                    {
                        InsertInternal(range.Minimum, text, semantics);
                    }                    
                }
        ...
        public bool ShouldProcessListParagraph(TextRange range, StyledText text, EditSemantics semantics, ref int paragraphIndex)
        {
            if (semantics != EditSemantics.ShiftPressed && _paragraphs.Count > 0 && (text.Length == 1 && text.CodePoints.First() == '\u2029' || text.Length == 0))
            {
                var position = text.Length == 1 ? range.Minimum : range.Maximum;
                paragraphIndex = GetParagraphForCodePointIndex(new CaretPosition(position), out var indexInParagraph);
                var paragraph = _paragraphs[paragraphIndex];         
                // if paragraph is empty stop numbering/bullet list
                return paragraph.ListKind != ListKind.None && paragraph.IsEmpty;
            }

            return false;
        }

        public void ToggleListParagraphs(TextRange range, ListKind listKind)
        {
            // Make sure layout is up to date
            Layout();
            ...
            if (range.IsRange)
            {
                var interectingRuns = Paragraphs.GetInterectingRuns(range.Start, range.Length).ToList();
                range1 = new Tuple<int, int>(interectingRuns.First().Index, interectingRuns.Last().Index);
            }
            else
            {
                var paraIndex = GetParagraphForCodePointIndex(new CaretPosition(range.Start), out var indexInParagraph);
                range1 = new Tuple<int, int>(paraIndex, paraIndex);
            }
            var rangeLength = range1.Item2 - range1.Item1 + 1;
            var paragraphsRange = _paragraphs.Skip(range1.Item1).Take(rangeLength).ToList();
            var numberedListsCount = paragraphsRange.Select(p => p.NumberedList).Distinct().Count();
            var bulletParagraphsCount = paragraphsRange.Count(p => p.ListKind == ListKind.BulletList);
            var numberedParagraphsCount = paragraphsRange.Count(p => p.ListKind == ListKind.NumberingList);
            var regularParagraphsCount = paragraphsRange.Count(p => p.ListKind == ListKind.None);

            var allBulletParagraphs = bulletParagraphsCount == rangeLength;
            var allNumberedParagraphs = numberedParagraphsCount == rangeLength && numberedListsCount == 1;
            var allRegularParagraphs = regularParagraphsCount == rangeLength;

            if ((allBulletParagraphs && listKind == ListKind.BulletList) || 
                allNumberedParagraphs)
            {
                using (_undoManager.OpenGroup((string) null))
                {
                    _undoManager.Do(new UndoCancelList(range1));
                    if (allNumberedParagraphs && listKind == ListKind.BulletList)
                    {
                        _undoManager.Do(new UndoCreateList(range1, listKind));
                    }
                }
            } else if(allRegularParagraphs || (allBulletParagraphs && listKind == ListKind.NumberingList))
            {
                _undoManager.Do(new UndoCreateList(range1, listKind));
            }
        ...
        internal void ClearNumberedListInfo(Paragraph paragraph)
        {
            paragraph.NumberedList?.Remove(paragraph);
            paragraph.NumberedList = null;
            paragraph.ListKind = ListKind.None;
        }
        ...
        int InsertInternal(int position, StyledText text, EditSemantics semantics)
        {
            ...
            if (parts.Length > 1)
            {
                ...
                paraB.FirstParaInList = semantics != EditSemantics.ShiftPressed;
                ...
                for (int i = 1; i < parts.Length - 1; i++)
                {
                    ...
                    betweenPara.FirstParaInList = semantics != EditSemantics.ShiftPressed;
                    var part = parts[i];
